...
object Routes {
    ...
    /** Safe URI parameter encoder that works both on Android and pure JVM test environments. */
    fun encodeUri(uriString: String): String =
        runCatching { Uri.encode(uriString) }.getOrNull()
            ?: java.net.URLEncoder.encode(uriString, "UTF-8").replace("+", "%20")

    fun browse(uri: Uri, selectAll: Boolean = false): String = browse(uri?.toString().orEmpty(), selectAll)
    ...
    fun player(uri: Uri, folderUri: Uri?): String = player(uri?.toString().orEmpty(), folderUri?.toString())
    ...
    fun image(uri: Uri, folderUri: Uri?): String = image(uri?.toString().orEmpty(), folderUri?.toString())
    fun image(uriString: String, folderUriString: String? = null): String = buildString {
        append("$IMAGE?$ARG_URI=${encodeUri(uriString)}")
        if (folderUriString != null) append("&$ARG_FOLDER=${encodeUri(folderUriString)}")
    }

    fun search(rootUri: Uri): String = search(rootUri?.toString().orEmpty())
